...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Recycle-back account.<br>
        ...
        &mdash; Recycle-back Team
      `
      return sendMail({ toEmail: email, subject: 'Recycle-back - Password Reset', content })
    })
